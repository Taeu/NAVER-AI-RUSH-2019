...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', type=int, default=2) # change
    ...
    if args.mode == "train":
        ...
        kf = KFold(n_splits = 2,random_state = random_seed,shuffle =True)
        image_dir = os.path.join(DATASET_PATH, 'train', 'train_data', 'images') 
        train_label_path = os.path.join(DATASET_PATH, 'train', 'train_label') 
        train_meta_path = os.path.join(DATASET_PATH, 'train', 'train_data', 'train_with_valid_tags.csv')
        ...
        for cvf in range(2):
            ...
            print(cvf)
            bind_model(model)            
            nsml.load(checkpoint='16',session='team_62/airush1/409')
