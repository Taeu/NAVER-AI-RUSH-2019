...
# dropout ratio  : 0.3 Ã¬ÂÂ¼Ã«Â¡Â Ã¬Â¡Â°Ã¬Â Â
    ...
    if args.mode == "train":
        ...
        for cvf in range(4):
            ...
            for epoch_idx in range(1,args.epochs +1):
                ...
                dataloader_valid = valid_dataloader(meta_data_val, args.input_size, args.batch_size, args.num_workers, label_matrix = label_val, transforms_i = transforms_i )
                scheduler.step()
                ...
                with torch.no_grad():
                    for batch_idx, (image, tags) in enumerate(dataloader_valid):
                        ...
                        output_prob_train = F.softmax(output, dim=1) #softmaxÃ«ÂÂ ÃªÂµÂ³Ã¬ÂÂ´ Ã­ÂÂÃ¬ÂÂÃ¬ÂÂÃªÂ³Â 
                ...
                print("valid__Accuracy {:2.4f}".format(total_correct/len(dataloader_valid)))
                print("valid__Loss {:2.4f}".format(total_loss_valid /len(dataloader_valid)))
